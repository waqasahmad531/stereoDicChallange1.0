...
for ven = 1: length(vend)
    ...
    for grp = 1:size(sysgroups,1)
        ...
        for iGroupNum = 1:size(groups,2)
            ...
            [fileNames, testDir,~, baseDir]=DicDataFileNames_v3(testingDir,groupNum);
            stepNames = strcat(fileNames,'_RegGrid.mat');

            % maindir = fullfile(testDir,stepNames);
    ...
    for grp = 1:size(sysgroups,1)
        ...
        for iGroupNum = 1:size(groups,2)
            ...
            for iFile = 1:size(stepNames,1)
                countr = 0;
                if  ven==1 && grp == 1 && iGroupNum == 1  && iFile == 1 %any(grpid==1) &&
                    load(frameNames(iFile));
                    Z = regGridData(:,:,3);
                        ...
                        if ven == 1 && grp == 1 && (iGroupNum == 1 || iGroupNum == 2)
                            ...
                            if avgFlag == 0
                                [fieldP,cLim,meanval(iFile,disp)] = fieldsAndResidualPlots(regGridData,Z,meanval(iFile,disp),...
                                    rmnan,iGroupNum,sbt,disp,testingDir,avgFlag,storeLim,fileext,...
...
for vn = 1:length(vend)
    ...
    for grp = 1:size(sysgroups,1)
        ...
        for cm = 1:4
            ...
            figfold = mfloc(end)-1;
            tm = fullfile(dirFolder,"colorbar\maincolorBar");%char(fieldP);
