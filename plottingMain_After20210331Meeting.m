             ...
             for avgFlag = 1
                ...
                for disp = 1 : 4
                   if ven == 1 && grp == 1 && (iGroupNum == 1 || iGroupNum == 2)
                      storeLim = true;
                      if avgFlag == 0
                         [fieldP,cLim,meanval(iFile,disp)] = post_plot_v5_After20210331Meeting(regGridData,Z,meanval(iFile,disp),...
                          rmnan,iGroupNum,sbt,disp,maindir,avgFlag,storeLim,fileext,...
                          stU,stV,stW,[cLimLo(iFile,disp),cLimHi(iFile,disp)],fixedmean,pltflag);
                         if iGroupNum ==  1
                         ...
                         [fieldP,cLim,meanval(iFile,disp),stat] = post_plot_v5_After20210331Meeting(regGridData,Z,meanval(iFile,disp),...
                         rmnan,iGroupNum,sbt,disp,maindir,avgFlag,storeLim,fileext,...
                         stU,stV,stW,[cLimAvgLo(iFile,disp),cLimAvgHi(iFile,disp)],fixedmean,pltflag);
...
for cm = 1:4
    ...
    clbr = sprintf('disp%g%s',cm,fileext);
