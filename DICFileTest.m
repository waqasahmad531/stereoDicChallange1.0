...
[fileName, fileDir] = uigetfile('*.mat','open DIC challenge file','*.mat');
inputFile = [fileDir fileName];
%get information about the data in the file
S=whos('-file', inputFile);
display(S.name);
%the data must have been in a single array called DICData 
if S.name(1) ~= 'DICData'
  f=uiwait(msgbox('The array in the file must be named DICData','Invalid File','modal'));
  return
end

%read in the data
load(inputFile);
%the data must have eight columns
if size(DICData,2)~=8
  f=uiwait(msgbox('The array in the file must have 8 columns','Invalid File','modal'));
  return
end  
...
xMin = min(DICData(:,3));
yMin = min(DICData(:,4));
DICData(:,3)=(DICData(:,3)-xMin)-50;
DICData(:,4)=(DICData(:,4)-yMin)-50;
xMax = max(DICData(:,3));
yMax = max(DICData(:,4));
xMin = min(DICData(:,3));
yMin = min(DICData(:,4));

%the data should fit onthe grid
if xMin<gridParms(1) || xMax>gridParms(2)||yMin<gridParms(1) || yMax>gridParms(2)
  disp(xMin)
  disp(xMax)
  disp(yMin)
  disp(yMax)
  f=uiwait(msgbox('The location values must be between -60 and 60','Invalid File','modal'));
  return
end  
  
%transform the data into a regular set of points
tic
disp('creating grid this will take a minute');
locData = DICData(:,3:5);
dispData = DICData(:,6:8);
gridData=DataGrid(locData, dispData, gridParms, minCounts);
toc

%display the Z values as an image, mask out areas without data
imgData = gridData(:,:,3);
imgData = imgData.';
imgData = flip(imgData);
imgMask = gridData(:,:,7);
imgMask = imgMask.';
imgMask = flip(imgMask);
im=image(imgData,'CDataMapping','scaled');
axis equal;
colormap('jet');
cmap=colormap;
im.AlphaData = imgMask;
colorbar;
