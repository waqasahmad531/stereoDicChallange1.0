function gridData = DataGrid2(locPts, dataPts, gridParms, minCounts)
...
%   drawnow
  ...
  qPoints(qPoints(:,5)<minCounts,:)=NaN;
  %remove points outside of the acceptable area
  qPoints(qPoints(:,1)<minX,:)=NaN;
  qPoints(qPoints(:,1)>maxX,:)=NaN;
  qPoints(qPoints(:,2)<minY,:)=NaN;
  qPoints(qPoints(:,2)>maxY,:)=NaN;
  ...
  v2=dataPts(:,2);
  v3=dataPts(:,3);
  noNanIdx = ~isnan(qPoints(:,1));
  
  %break out qPoints
  xq=qPoints(noNanIdx,1);
  yq=qPoints(noNanIdx,2);
  idxI=qPoints(noNanIdx,3);
  idxJ=qPoints(noNanIdx,4);
  nPnts=qPoints(noNanIdx,5);
